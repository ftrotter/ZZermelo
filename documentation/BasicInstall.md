...
config. The database user will need CREATE TABLE permissions in order to create the \_zzermelo database (or if you are 
...
1. From the command prompt at your laravel **project's root** install the following commands: 
    ...
    ```
    composer require careset/zzermelo_installer
    php artisan zzermelo:install
    ```
    
The installer pulls down the the zzermelo package, which contains the API backend, and all avaialble view packages, 
such as zzermelobladetabular and zzermelobladecard. The installer will also move zzermelo assets such as Javascript and 
...
The installer will also create a _zzermelo_cache databse and a _zzermelo_config database. The cache
