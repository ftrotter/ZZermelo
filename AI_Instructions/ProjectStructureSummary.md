# ZZermelo Project Structure Summary

This document provides a comprehensive overview of the ZZermelo reporting engine architecture, core workflows, and relationship with the LoreCommander test project.

---

## Project Overview

**ZZermelo** is a PHP/Laravel reporting engine that transforms SQL SELECT statements into interactive web-based reports. The core philosophy is "SQL-first" - report authors think exclusively in SQL, and ZZermelo handles all the complexity of web rendering, pagination, filtering, sorting, and caching.
...
│   │   ├── ZZermeloReport.php  # Base report class
...
The caching system is central to ZZermelo's performance and functionality.

### Location

- **File**: `src/Models/DatabaseCache.php`
- **Cache Database**: Configured as `_zzermelo_cache` by default
...
- **URL**: `/ZZermeloTreeCard/ReportName/`
...
**LoreCommander** is a Laravel project that uses ZZermelo to report on Magic: The Gathering card data from Scryfall.

### Purpose

- Exercises ZZermelo with real and test reports
- Validates that ZZermelo continues to function correctly
...
Files prefixed with `DURC_` are auto-generated by the DURCC (Database Utility Report Creator) sister project. DURCC generates ZZermelo reports for each table in a database. For ZZermelo development, these should be treated as "just more reports" - ZZermelo must function independently of DURCC.
...
| Endpoint | Purpose |
|----------|---------|
| `/ZZermelo/{report}` | Tabular report web view |
| `/ZZermeloCard/{report}` | Card report web view |
| `/ZZermeloGraph/{report}` | Graph report web view |
| `/ZZermeloTreeCard/{report}` | TreeCard web view |
| `/api/ZZermelo/{report}` | Tabular JSON API |
| `/api/ZZermelo/{report}/Summary` | Summary statistics |
| `/api/ZZermelo/{report}/Download` | CSV/Excel download |
| `/ZZermeloSQL/{report}` | SQL debug view (if enabled) |
...
public function MapRow(array $row, int $row_number): array {
    $id = $row['id'];
    $name = $row['name'];
    $row['name'] = "<a href='/ZZermelo/DetailReport/$id'>$name</a>";
    return $row;
}
```
